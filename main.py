...
def main(_):
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        if i <= FLAGS.offline_steps:
            ...
            # --------------------------------------------------------------
            # Heavy metrics (policy sample + advantage weights) –
            # выполняем ТОЛЬКО на шагах логирования, иначе пропускаем, чтобы
            # не тормозить обучение из-за host/device synchronisation.
            # --------------------------------------------------------------
            policy_actions = None
            weights = None
            if (i % FLAGS.log_interval) == 0:
                try:
                    sample_obs = batch['observations'][:32]  # small subset for metrics
                    ...
                    if config.get('advantage_weighted', False):
                        ...
                        use_min = hasattr(agent.config, 'q_agg') and agent.config.get('q_agg') == 'min'
                        q = q_values.min(axis=0) if use_min else q_values.mean(axis=0)
                        # IFQL uses separate value network, FQL approximates with mean Q
                        ...
                        weights = jnp.clip(jnp.exp(beta * advantage), 0.1, 10.0)
                except Exception:
                    policy_actions = None
                    weights = None
